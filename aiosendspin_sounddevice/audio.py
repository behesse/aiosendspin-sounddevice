...
class AudioPlayer:
    ...
    def get_volume(self) -> tuple[int, bool]:
        ...
        Returns:
            Tuple of (volume, muted) where volume is 0-100.
    ...
    def _audio_callback(  # noqa: PLR0915, C901, PLR0912
        self,
        outdata: memoryview,
        frames: int,
        time: AudioTimeInfo,
    ...
    def _update_correction_schedule(self, error_us: int) -> None:
        ...
        if (
            ...
            logger.debug("Sync error %.1f ms too large; re-anchoring", abs_err / 1000.0)
            # Reset cadence
            self._insert_every_n_frames = 0
            self._drop_every_n_frames = 0
            self._frames_until_next_insert = 0
    ...
    def submit(self, server_timestamp_us: int, payload: bytes) -> None:  # noqa: PLR0915, C901, PLR0912
        ...
        if self._clear_requested:
            self._clear_requested = False
            self.clear()
            logger.debug("Cleared audio queue after underflow (deferred from audio thread)")
